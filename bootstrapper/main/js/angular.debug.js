...
(function (window, document, undefined) {
    ...
    var debugOptions = {
        digest: {
            ...
            aggregatedTime: false, // true, 500, false
            watchLapseTime: false, // true, 30, false
            watchFnLapseTime: false, // true, 30, false
            watchPerformance: true // true, false
        }
    };
        ...
        this.$get = ['$injector', '$exceptionHandler', '$parse', '$browser',
            function ($injector, $exceptionHandler, $parse, $browser) {
                ...
                Scope.prototype = {
                    ...
                    $digest: function () {
                        // [MFS] [debug::digest]
                        var startTime, endTime, timer, newTimer, lapseTime;
                        var watchStartTime, watchEndTime, watchFnStartTime, watchFnEndTime;
                        var watchLapseTime, watchFnLapseTime;

                        var watchPerformance, watchesPerformance;
                        if (debugOptions.digest.enabled) {
                            timer = startTime = performance.now();
                            console.warn('digest started: ' + numeral(startTime / 1000).format('0,0.000'));
                            endTime = newTimer = lapseTime = null;
                            watchStartTime = watchEndTime = watchFnStartTime = watchFnEndTime = watchLapseTime = watchFnLapseTime = null;
                            watchesPerformance = [];
                        }
                                    ...
                                    if ((watchers = current.$$watchers)) {
                                        ...
                                        while (length--) {
                                            try {
                                                ...
                                                watchPerformance = {};
                                                watchPerformance.exp = watch.exp;
                                                watchPerformance.fn = watch.fn;
                                                // Most common watches are on primitives, in which case we can short
                                                // circuit it with === operator, only when === fails do we use .equals
                                                if (watch) {
                                                    // [MFS] [debug::digest] Log start time of the watch evaluation
                                                    if (debugOptions.digest.enabled) {
                                                        watchStartTime = watchEndTime = watchFnStartTime = watchFnEndTime = watchLapseTime = watchFnLapseTime = null;
                                                    ...
                                                    if ((value = watch.get(current)) !== (last = watch.last) &&
                                                        ...
                                                        if (debugOptions.digest.enabled) {
                                                            // [MFS] [debug::digest] Log end time of the watch evaluation
                                                            watchEndTime = performance.now();
                                                            // [MFS] [debug::digest] Setting watch evaluation lapse time
                                                            watchLapseTime = watchEndTime - watchStartTime;
                                                            watchPerformance.lapseTime = watchLapseTime;
                                                            // [MFS] [debug::digest] Voice out if it uses more than x millisecond
                                                            if (debugOptions.digest.watchLapseTime === true ||
                                                                (debugOptions.digest.watchLapseTime && watchLapseTime > debugOptions.digest.watchLapseTime)) {
                                                                console.warn('The following watch spent ' + numeral(watchLapseTime / 1000).format('0.000') + ' to eval:');
                                                                console.warn(watch);
                                                            }
                                                        }
                                                        dirty = true;
                                                        ...
                                                        if (debugOptions.digest.enabled) {
                                                            // [MFS] [debug::digest] Setting watch function start time
                                                            watchFnStartTime = performance.now();
                                                        }
                                                        watch.fn(value, ((last === initWatchVal) ? value : last), current);
                                                        if (debugOptions.digest.enabled) {
                                                            // [MFS] [debug::digest] Setting watch function end time
                                                            watchFnEndTime = performance.now();
                                                            // [MFS] [debug::digest] Setting watch function lapse time
                                                            watchFnLapseTime = watchFnEndTime - watchFnStartTime;
                                                            watchPerformance.fnLapseTime = watchFnLapseTime;
                                                            watchesPerformance.push(watchPerformance);
                                                            // [MFS] [debug::digest] Voice out if it uses more than x millisecond
                                                            if (debugOptions.digest.watchFnLapseTime === true ||
                                                                (debugOptions.digest.watchFnLapseTime && watchFnLapseTime > debugOptions.digest.watchFnLapseTime)) {
                                                                console.warn('The following watch spent ' + numeral(watchFnLapseTime / 1000).format('0.000') + ' to execute:');
                                                                console.warn(watch);
                                                            }
                                                        }
                        ...
                        if (debugOptions.digest.enabled) {
                            // [MFS] [debug::digest] Voice out if it exceeds threshold time
                            endTime = performance.now();
                            lapseTime = endTime - startTime;
                            if (debugOptions.digest.aggregatedTime === true ||
                                (debugOptions.digest.aggregatedTime && lapseTime > debugOptions.digest.aggregatedTime)) {
                                console.warn('digest ended: ' + numeral(endTime / 1000).format('0,0.000') + ', aggregated time: ' + numeral(lapseTime / 1000).format('0,0.000'));
                            } else {
                                // do nothing
                            }
                            // [MFS] [debug::digest] watch perfomrance:
                            if (debugOptions.digest.watchPerformance) {
                                console.warn('watch performance: ');
                                console.warn(watchesPerformance.reduce(function (previousValue, currentValue, index, array) {
                                    if (previousValue.zzTotalWatchCount) {
                                        previousValue.zzTotalWatchCount++;
                                    } else {
                                        previousValue.zzTotalWatchCount = 1;
                                    }
                                    if (currentValue.fnLapseTime) {
                                        if (previousValue.zzTotalWatchChangedCount) {
                                            previousValue.zzTotalWatchChangedCount++;
                                        } else {
                                            previousValue.zzTotalWatchChangedCount = 1;
                                        }
                                        if (previousValue[currentValue.exp]) {
                                            previousValue[currentValue.exp]++;
                                        } else {
                                            previousValue[currentValue.exp] = 1;
                                        }
                                    }
                            ...
                            console.warn(watchesPerformance.reduce(function (previousValue, currentValue, index, array) {
                                if (currentValue.lapseTime <= 10) {
                                    previousValue['eval0to10']++;
                                } else if (currentValue.lapseTime <= 50) {
                                    previousValue['eval11to50']++;
                                } else if (currentValue.lapseTime <= 100) {
                                    previousValue['eval51to100']++;
                                } else if (currentValue.lapseTime <= 500) {
                                    previousValue['eval101to500']++;
                                } else {
                                    previousValue['eval501']++;
                                }
                                if (!currentValue.fnLapseTime) {
                                    previousValue['fn0']++;
                                } else if (currentValue.fnLapseTime <= 10) {
                                    previousValue['fn1to10']++;
                                } else if (currentValue.fnLapseTime <= 50) {
                                    previousValue['fn11to50']++;
                                } else if (currentValue.fnLapseTime <= 100) {
                                    previousValue['fn51to100']++;
                                } else if (currentValue.fnLapseTime <= 500) {
                                    previousValue['fn101to500']++;
                                } else {
                                    previousValue['fn501']++;
                                }
                                return previousValue;
